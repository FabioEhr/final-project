...
{
  Cell P_cell;
  int condition;      // susceptible=0, infected=1, recovered=2, incubating
                      // infection=3
  int incub_day = 0;  // counter for days of incubation

  Person(int m_r = 1, int m_c = 1, int m_cond = 0)
  ...
  {  // fa muovere una persona in modo random data una velocità e i limiti di
     // posizione
    ...
    // boundaries (walls) questo fa sì che se le persone uscissero dai muri
    // rimangano sui muri.
...
struct Grid
{
    ...
    if (inf_cells.begin() != inf_cells.end()) {
      for (Person& sus_person : people) {
        if (sus_person.condition == 0 &&
            (inf_cells.find(sus_person.P_cell) !=
          ...
          if ((grid_evolve_distr2(grid_evolve_r2)) <= e_virus.contagiousness) {
            ...
            std::cout << "person " << person_index << " infected in cell ("
                      << sus_person.P_cell.r << "," << sus_person.P_cell.c
  ...
  void move_and_evolve(int mobility, int grid_speed, Virus mne_virus)
  {
    ...
    for (Person& inf_or_incub : people) {
      ...
      } else if (inf_or_incub.condition ==
                 3) {  // implemented incubation so that the people that get

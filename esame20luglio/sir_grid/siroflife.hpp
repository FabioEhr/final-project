...
class Person
{
  ...
  {  // makes a person move randomly, given a certain speed and grid limits
    ...
    //this creates a bias towards staying on the borders when the grid is small and the speed is high
...
class Grid
{
      ...
      , recovered{m_rec}
  {
    ...
    for (int i = 0; i < population; ++i) { // taking all the people from (1,1) to a random starting position
      
      // (people[i]).r = a_random_number between 1 and  height
      // (people[i]).c = a_random_number between 1 and  width
      people[i].Set_P_Cell(grid_distr_r(generator_grid), grid_distr_c(generator_grid));
    }
  }
    ...
    if (susceptible != 0 && inf_cells.begin() != inf_cells.end()) {
      ...
      for (Person& sus_person : people) {
        if (sus_person.Get_Condition() == PersonState::Susceptible &&
          (inf_cells.find(sus_person.Get_P_Cell()) !=inf_cells.end())) {  // find a sus person in an infected cell.
          
          // find returns an iterator to the end of the vector if it doesn't find anything
          ...
          // gets infected)
          if ((sus_evolve_distr(generator_sus_evolve)) <= e_virus.contagiousness) {
  ...
  {
    for (int i = 0; i < mobility; ++i) {  // mobility indicates how many times in a day each person moves
    ...
    for (Person& inf_or_incub : people) {
      ...
      if (inf_or_incub.Get_Condition() == PersonState::Infected) {
        ...
        if (grid_evolve_distr(generator_grid_evolve) <= mne_virus.recovery_rate) {

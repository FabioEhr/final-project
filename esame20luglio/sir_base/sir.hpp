      ...
      , time{tim}
  {
    ...
    if (0 > infected || infected > population) {
      throw std::runtime_error{
          "The number of infected must be between 0 and population  "};
    }
    if (0. > suscettibles || suscettibles > population) {
      throw std::runtime_error{
          "The number of suscettibles must be between 0 and population  "};
    }
    if (0. > recovered || recovered > population) {
      throw std::runtime_error{
          "The number of recovered must be between 0 and population  "};
    }
    if (!(population - 0.01 < infected + suscettibles + recovered &&
          infected + suscettibles + recovered < population + 0.01)) {
      throw std::runtime_error
...
struct Virus
{
  ...
  Virus(double const cont = 0., double const rec = 0.)
      : contagiousness{cont}, recovery_rate{rec}
  {
...
class Pandemic
{
 ...
 public:
  ...
  void evolve()
  {
    ...
    next.infected = condition.infected +

                    virus.contagiousness * condition.infected *
                        condition.suscettibles / condition.population -
    ...
    condition = next;
    assert(
        condition.population - 0.01 <
            condition.infected + condition.recovered + condition.suscettibles &&
        condition.population + 0.01 >
            condition.infected + condition.recovered + condition.suscettibles);
  }

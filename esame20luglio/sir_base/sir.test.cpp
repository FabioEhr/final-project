...
TEST_CASE("Testing different contagiousness")
{
  ...
  CHECK(d_comp(test_1.Get_condition().infected +
                   test_1.Get_condition().suscettibles +
                   test_1.Get_condition().recovered,
               0.99,
               1.01));
  CHECK(d_comp(test_2.Get_condition().infected +
                   test_2.Get_condition().suscettibles +
...
TEST_CASE("Testing that percentages always add up to 1")
{
  ...
  CHECK(d_comp(test_2.Get_condition().infected +
               ...
               0.99,
               1.01));
  CHECK(d_comp(test_3.Get_condition().infected +
                   test_3.Get_condition().suscettibles +
                   test_3.Get_condition().recovered,
               0.99,
               1.01));
  CHECK(d_comp(test_4.Get_condition().infected +

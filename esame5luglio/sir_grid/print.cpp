...
int incubation;
  while(true){
      ...
      std::cout<< "Incubation time has been set to "<< incubation << " Type y to continue, n to change incubation time."<< '\n';
      char decision;
      std::cin>>decision;
      if(decision == 'y'){break;}
      else {continue;}
   ...
   while(true){
      ...
      if(height>=1){break; std::cout<< "Grid height has been set to "<< height << '\n';}
      else {continue;}
   }
   int width=1;
   while(true){
      ...
      if(width>=1){break; std::cout<< "Grid width has been set to: "<< width << '\n';}
      else {continue;}
   }
   int sus=0;
   int inf=0;
   ...
   int speed=0;
       ...
       if(mob<1) {mob=1; std::cout<< "Mobility has been set to the minimum value of 1"<<'\n';}
       else {std::cout<< "Mobility has been set to "<< mob <<'\n';}
        std::cout<< "How many maximum squares should people be able to move for in each direction?" <<'\n';
       std::cin>> antibug;
       speed=string_to_int(antibug);
       if(speed<1) {speed=1; std::cout<< "Speed has been set to the minimum value of 1"<<'\n';}
       else {std::cout<< "Speed has been set to "<< speed <<'\n';}

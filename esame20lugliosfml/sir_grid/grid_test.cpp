...
TEST_CASE("Testing incubation")
{
  ...
  CHECK(static_cast<int>(test.Get_People()[0].Get_Condition()) +
            static_cast<int>(test.Get_People()[1].Get_Condition()) ==
        3);
  CHECK(test.Get_Population() == 2);
  test.move_and_evolve(1, 1, vir);
  CHECK(static_cast<int>(test.Get_People()[0].Get_Condition()) +
...
TEST_CASE("Testing boundries")
{
  ...
  CHECK(test.Get_Population() == 2);
  CHECK(test.Get_Infected() ==
        2);  // since they should stop on boundries, with 100 moves the prob of

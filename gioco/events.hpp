...
{ 
    ...
    std::cout<< "On the local newspaper " << news_paper() << " there's an article titled " << variant() << '\n';
    std::cout<< "An extract from that article reads: 'Some experts believe that the virus is getting more contagious each day.'" << '\n';      
    
}
void mutate_recovery( City& playground){ //g=gamma
    double amount= -0.005;
    Virus plague=playground.Get_virus();
    plague.g += amount;
    playground.Set_virus(plague);
    if(plague.g<0.) {plague.g =0.;} 
    playground.add_know(-2);
    std::cout<< "On the local newspaper " << news_paper() << " there's an article titled " << variant() << '\n';
    std::cout<< "An extract from that article reads: 'A new mutation in the virus increases recovery time.'" << '\n';
}
...
{double amount= 0.005;
 Virus plague=playground.Get_virus();
    ...
    playground.add_know(-2);
    std::cout<< "On the local newspaper " << news_paper() << " there's an article titled " << variant() << '\n';
    std::cout<< "An extract from that article reads: '...this slight alteration in the virus' genetic sequence, might increase its deadliness.'" << '\n';
}
...
void illegal_parties(City & playground){
    ...
    std::cout<< "On the local newspaper " << news_paper() << " there's an article titled 'Spike in contagion following illegal parties'" << '\n';
    std::cout<< "An extract from that article reads: 'Irresponsability might be the leading cause of death this year."<< '\n'; 
}

void eureka(City & playground){
    playground.add_know(3);
    std::cout<< "On the local newspaper " << news_paper() << " there's an article titled 'Brilliant scientists bring us closer to a vaccine.'" << '\n';
    std::cout<< "An extract from that article reads: '[...] the sequencing of this gene may reveal a new way to produce a vaccine."<< '\n';
}
...
void panic(City & playground)
{
    ...
    other_el.morale += -2;
    playground.Set_ages(other_young, other_adu, other_el);
     std::cout<< "On the local newspaper " << news_paper() << " there's an article titled 'Panicked people storm supermarkets.'" << '\n';
...
void apocalypse_now(City &playground){
    ...
    other_el.morale += -6;
    playground.Set_ages(other_young, other_adu, other_el);
    std::cout<< "On the local newspaper " << news_paper() << " there's an article titled 'No country for old men." << '\n';
    std::cout<< "An extract from that article reads: 'People are currently dying outside of hospitals, due to the inability of the government to provide hospital beds. "<< '\n';
}
...
void rnd_events(City &playground)
{
 state_function current= playground.Get_status();   
    std::random_device r1;
       ...
       if(!current.restaurants || !current.theatres|| !current.churches|| !current.schools) 
       {
           if(event<7) {illegal_parties(playground);}
       }
     }
       }

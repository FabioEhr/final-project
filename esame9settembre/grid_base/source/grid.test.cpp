...
TEST_CASE("Testing all sucettibles")
{
  World block{3};
  CHECK(std::all_of(
      block.getGrid().begin(), block.getGrid().end(), [](const Cell& actual) {
        return actual == Cell::Suscettible;
      }));
  Virus virus{1., 0.};
  ...
  CHECK(std::all_of(
      block.getGrid().begin(), block.getGrid().end(), [](const Cell& actual) {
        return actual == Cell::Infected;
      }));
}
TEST_CASE("Testing all infected")
{
  World block{4};
  grid_base::insertGrid(
      block, block.getSide_length() * block.getSide_length(), 0);
  CHECK(std::all_of(
      block.getGrid().begin(), block.getGrid().end(), [](const Cell& actual) {
        return actual == Cell::Infected;
      }));
...
TEST_CASE("Testing all recovered")
{
  World block{4};
  grid_base::insertGrid(
      block, 0, block.getSide_length() * block.getSide_length());
  CHECK(std::all_of(
      block.getGrid().begin(), block.getGrid().end(), [](const Cell& actual) {
        return actual == Cell::Recovered;
      }));
...
TEST_CASE("Quick propagation")
{
  ...
  for (int i = 1; i < N; i += 2) {
    ...
    CHECK(std::count(block.getGrid().begin(),
                     block.getGrid().end(),

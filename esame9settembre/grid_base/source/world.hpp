  ...
  Virus(double cont, double rec) : contagiousness{cont}, recovery_rate{rec}
  {
    if (!d_comp(contagiousness, 0., 1.))
      throw std::runtime_error{
          "contagiousness has to be set to a value between 0 and 1"};
...
class World
{
 ...
 public:
  World(int const len) : side_length{len}
  {
    if (side_length < 1)
      throw std::runtime_error{
          "the length of the side has to be a positive value"};
  ...
  {
    // attraverso un operatore ternario esprimo il fatto che se chiedo lo stato
    // di una cella fuori dal vettore questa risulterà recovered in modo che non
    // influisca
  ...
  {
    // trasforma sum in una coppia di coordinate in modo da chiamare il cell
    // precedente

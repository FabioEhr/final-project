...
void City::add_mob(double const yy,
                   double const aa,
                   double const ee,
                   double const ya,
                   double const ye,
...
// replacing functions
void City::Set_ages(Age const& other_young,
...
void City::evolve()
{
  ...
  y.rec +=
      vir.g * current_young_inf + (vir.g + h.r_chance_mod) * current_young_hosp;
  ...
  e.rec +=
      vir.g * current_elder_inf + (vir.g + h.r_chance_mod) * current_elder_hosp;
...
void City::evolve_n_print()
{
  ...
  std::cout << "Apparent rounding error: "
            << delta_inf - delta_y - delta_a - delta_e << '\n';
  // this apparent rounding error is irrelevant because we always use
  // percentages for calculations. the discrepancy is due to the way the compiler
  // rounds integers from double*ints operations the displayed value is the most
  // accurate one, because in the Delta sum the compiler rounds 3 times, while in
  // the displayed one there is only one rounding. Still for testing and
  // calibration it's usefull to have a rough estimate on how many
  // young/adults/elders are infected.
  std::cout << '\n';
  // saving the current percentages of infected, since we have to
  // do multiple calculations based on them
  double current_young_inf = y.inf;
